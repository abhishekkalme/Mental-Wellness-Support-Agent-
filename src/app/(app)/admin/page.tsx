      ...
      <section className="flex-1 overflow-y-auto p-10 relative z-10 mx-auto w-full max-w-3xl custom-scroll">
        <div className="space-y-3 mb-8">
          ...
          <p className="text-sm text-white/50 leading-relaxed max-w-xl">
            Switch your primary AI Provider and Model logic dynamically. You must be authenticated
            to modify system-level environment paths.
          </p>
        </div>
        ...
        {settings ? (
          <div className="rounded-[32px] border border-white/5 bg-white/5 p-8 shadow-2xl backdrop-blur-md space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="space-y-4">
              ...
              >
                <option value="gemini" className="bg-[#0A0D08]">
                  Gemini Core (Free-Tier API)
                </option>
                <option value="openrouter" className="bg-[#0A0D08]">
                  OpenRouter Backbone (Production)
                </option>
                <option value="ollama" className="bg-[#0A0D08]">
                  Ollama Node (Local Dedicated)
                </option>
              </select>
            ...
            <div className="space-y-4">
              ...
              <input
                ...
                defaultValue=""
                placeholder={
                  settings.apiKey ? `[MASKED] ${settings.apiKey}` : 'Install new security token...'
                }
            ...
            <div className="pt-6 mt-6 border-t border-white/5 flex items-center justify-between">
              <div className="flex flex-col gap-1">
                ...
                <span className="text-xs text-white/70 font-mono bg-white/5 px-2 py-1 rounded-md">
          ...
          <div className="mt-8 flex items-center justify-center p-10 border border-dashed border-white/10 rounded-3xl bg-white/[0.01]">
            <span className="text-white/30 text-sm font-bold uppercase tracking-widest">
              {loading ? (
                <>
                  <RefreshCw className="w-4 h-4 animate-spin inline mr-2" /> Encrypting link...
                </>
              ) : (
                'Awaiting Security Matrix'
              )}
            </span>
          </div>
        )}
