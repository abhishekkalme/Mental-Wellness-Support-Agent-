...
const seedGroups = [
  { name: "Anxiety Support", members: "1.2k", active: "45", category: "Mental Health", icon: "🌱", joined: true },
  { name: "Study Stress & Burnout", members: "850", active: "12", category: "Academic", icon: "📚", joined: false },
  { name: "Loneliness & Connection", members: "2.1k", active: "120", category: "Social", icon: "🤝", joined: false },
  { name: "ADHD Strategies", members: "640", active: "8", category: "Neurodiversity", icon: "⚡", joined: true },
  { name: "Gratitude Daily", members: "3.4k", active: "200", category: "Wellness", icon: "🙏", joined: false },
];

const seedPosts = [
  { user: "Anonymous Panda", time: "2h ago", content: "Had a really rough day today with exam prep, but I managed to finish one chapter. Small wins!", likes: 24, comments: 5, liked: false },
  { user: "Kind Soul", time: "4h ago", content: "Does anyone have tips for winding down after a 10pm lecture? My brain stays wired for hours.", likes: 12, comments: 18, liked: false },
  { user: "Night Owl", time: "5h ago", content: "Just sharing some positive vibes to whoever is reading this. You are doing great, and things will get better. ❤️", likes: 89, comments: 12, liked: true },
];
  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-muted-foreground animate-pulse font-medium">Connecting to community...</p>
      </div>
    );
  }
      ...
      <main className="p-8 max-w-7xl mx-auto min-h-[70vh] flex items-center justify-center">
        <div className="glass-panel p-12 text-center flex flex-col items-center gap-6 max-w-md bg-secondary/10 border-sky-400/20">
          ...
          <div>
            ...
            <p className="text-muted-foreground">The community module is currently empty in our database. Click "Create" to initialize the safe space.</p>
          </div>
          <Button onClick={handleCreateInitialData} size="lg" className="rounded-xl px-12 h-14 text-lg font-bold shadow-lg shadow-sky-400/20 bg-sky-500 hover:bg-sky-600">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-1 space-y-6">
           <div className="glass-panel p-6 space-y-4">
              <h3 className="font-bold text-sm uppercase tracking-widest text-muted-foreground">My Joined Groups</h3>
              <div className="space-y-2">
                {groups.filter(g => g.joined).map(g => (
                  <button key={g._id || g.id} className="w-full text-left p-3 rounded-xl hover:bg-secondary/50 transition-all flex items-center gap-3 group">
                ...
                {groups.filter(g => g.joined).length === 0 && (
                  <p className="text-xs text-muted-foreground text-center py-4">You haven't joined any groups yet.</p>
                )}
              </div>
           </div>
        ...
        <div className="lg:col-span-3 space-y-8">
           <div className="flex items-center justify-between border-b border-border">
              ...
              <div className="pb-4 relative hidden md:block">
                 ...
                 <input type="text" placeholder="Search community..." className="pl-9 pr-4 py-1.5 rounded-full text-sm bg-secondary/50 border-none outline-none focus:ring-1 ring-primary/50" />
              </div>
           </div>
           ...
           {activeTab === "Explore" && (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {groups.map(g => (
                  <div key={g._id || g.id} className="glass-panel p-6 hover:border-sky-400/30 transition-all cursor-pointer group flex flex-col justify-between">
                     <div>
                     ...
                     <div className="mt-6 pt-4 border-t border-border flex items-center justify-between">
                        ...
                        <Button onClick={(e) => handleJoinGroup((g._id || g.id)!, e)} size="sm" variant={g.joined ? "primary" : "ghost"} className="text-xs font-bold gap-1">
           ...
           {activeTab === "Recent Threads" && (
              <div className="space-y-4">
                 {posts.map(p => (
                   <div key={p._id || p.id} className="glass-panel p-8 space-y-6 hover:border-border transition-colors">
                      <div className="flex items-center justify-between">
                         <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-lg">{p.user === "Anonymous Panda" ? "🐼" : p.user === "Hidden Fox" ? "🦊" : "🦋"}</div>
                            <div>
                      ...
                      <p className="text-foreground/80 leading-relaxed text-sm md:text-base">{p.content}</p>
                      <div className="flex items-center gap-6 pt-2">
                         <button onClick={() => handleLikePost((p._id || p.id)!)} className={`flex items-center gap-2 text-xs font-bold transition-colors ${p.liked ? "text-rose-500" : "text-muted-foreground hover:text-primary"}`}>
                            <Heart className={`w-4 h-4 ${p.liked ? "fill-rose-500" : ""}`} /> {p.likes} Likes
                         </button>
           ...
           {activeTab === "Saved" && (
              <div className="glass-panel p-12 text-center flex flex-col items-center gap-4">
                 ...
                 <div>
                    ...
                    <p className="text-muted-foreground text-sm max-w-sm mx-auto">Threads you save for later will appear here. Start exploring and save helpful advice.</p>
                 </div>
                 <Button onClick={() => setActiveTab("Explore")} variant="outline" className="mt-4">Go to Explore</Button>
              </div>
           )}
        </div>
      </div>
            ...
            >
               ...
               <div className="space-y-4">
                 ...
                 <div className="flex items-center justify-between">
                    ...
                    <Button onClick={handleCreatePost} disabled={!newPostContent.trim()} className="gap-2">
