...
export default function CrisisPage() {
  ...
  const handleCreateInitialData = async () => {
    ...
    try {
      ...
      console.error('Failed to seed helplines', err);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="glass-panel p-12 text-center flex flex-col items-center gap-6 max-w-md bg-secondary/10 border-rose-400/20">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Emergency helpline data is missing from our database. Click &quot;Create&quot; to
              initialize these life-saving resources.
            </p>
          </div>
          <Button
            onClick={handleCreateInitialData}
            size="lg"
            className="rounded-xl px-12 h-14 text-lg font-bold shadow-lg shadow-rose-400/20 bg-rose-500 hover:bg-rose-600"
          >
  ...
  return (
    <main
      ...
      role="main"
      aria-label="Crisis support and emergency resources"
    >
      <header className="flex items-center gap-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-lg">
            Immediate support for when things feel heavy.
          </p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <section className="glass-panel p-8 bg-rose-500/5 border-rose-500/20 space-y-6">
            ...
            <div className="space-y-4">
              {helplines.map((h, i) => (
                <div
                  key={h._id || i}
                  className="flex items-center justify-between p-4 rounded-2xl bg-white border border-rose-100 shadow-sm"
                >
                  <div>
                  ...
                  {h.phone ? (
                    <a
                      ...
                      className="h-10 px-4 flex items-center bg-rose-500 text-white rounded-xl font-bold text-xs md:text-sm hover:bg-rose-600 transition-colors whitespace-nowrap"
                    >
                      Call {h.phone}
                    </a>
                  ) : (
                    <a
                      ...
                      aria-label={`Visit ${h.name} website`}
                      className="h-10 px-4 flex items-center border border-rose-500 text-rose-500 rounded-xl font-bold text-xs md:text-sm hover:bg-rose-50 transition-colors whitespace-nowrap"
                    >
                      {h.country === 'WhatsApp' ? (
              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="Find nearest hospital on map"
                ...
                >
                  <Mail className="w-6 h-6 mb-2" aria-hidden="true" />
                  <span className="font-bold text-sm">Email Support</span>
                </a>
              </div>
            </div>
            <p className="text-xs text-muted-foreground text-center">
              If you are in immediate physical danger, please call your local emergency services
              (e.g., 911, 999).
            </p>
          </section>
          ...
          <section className="glass-panel p-8 space-y-6">
            ...
            <p className="text-muted-foreground italic">
              &quot;I am breathing in, I am breathing out. I am safe in this moment.&quot;
            </p>
            <div className="space-y-4">
              {[
                { name: '5-4-3-2-1 Rule', sub: 'Name 5 things you see, 4 you can touch...' },
                {
                  name: 'The Butterfly Hug',
                  sub: 'Cross your arms and tap your shoulders rhythmically.',
                },
                { name: 'Cold Water', sub: 'Splash your face or hold an ice cube.' },
              ].map((t, i) => (
                <div
                  key={i}
                  className="flex gap-4 p-4 rounded-xl hover:bg-secondary/50 transition-colors border border-transparent hover:border-border"
                >
                  <div className="w-10 h-10 rounded-full bg-emerald-500/10 text-emerald-600 flex items-center justify-center shrink-0 font-bold">
                    {i + 1}
                  </div>
                  <div>
        ...
        <div className="space-y-8">
          <div className="glass-panel p-8 space-y-8 bg-primary/5 border-primary/20">
            ...
            <div className="space-y-6">
              <p className="text-sm text-muted-foreground leading-relaxed">
                Imagine a place where you feel completely safe. It can be a forest, a beach, or a
                room. Visualize the colors, the smells, and the temperature.
              </p>
          ...
          <div className="glass-panel p-8 space-y-6">
            ...
            <div className="grid gap-3" role="list" aria-label="Calming audio tracks">
              {[
                'White Noise Stream',
                'Soft Rain Backdrop',
                'Guided Reassurance',
                ...
                <button
                  ...
                  aria-label={playingId === i ? `Stop ${track}` : `Play ${track}`}
                  aria-pressed={playingId === i}
                ...
                >
                  <span className="font-medium text-sm">
                    {playingId === i ? `Playing ${track}...` : track}
                  </span>
                  {playingId === i ? (
                    <Headphones
