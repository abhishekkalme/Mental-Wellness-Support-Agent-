...
export default function CrisisPage() {
  ...
  const [isAlertToastVisible, setAlertToastVisible] = useState(false);
  ...
  const [breathingPhase, setBreathingPhase] = useState<'idle' | 'inhale' | 'hold' | 'exhale'>(
    'idle'
  );
  ...
  const openAlertModal = () => {
    if (personalContacts.length === 0) {
      setAlertToastVisible(true);
      setTimeout(() => setAlertToastVisible(false), 3000);
  ...
  if (status === 'loading' || dbLoading) {
    return (
      <div className="min-h-screen bg-[#0A0C0B] flex items-center justify-center text-white/60 font-bold">
        Loading...
      </div>
    );
  }
  ...
  return (
    <main id="main-content" className="min-h-screen bg-[#0A0C0B] text-white pb-24">
      {activeModule ? (
        <div className="min-h-screen p-6 flex flex-col items-center justify-center relative overflow-hidden">
          <div
            className={`absolute inset-0 bg-gradient-to-br ${activeModule.gradient} opacity-30`}
          />
          ...
          <div className="relative z-10 w-full max-w-2xl space-y-10 text-center px-4">
            <div className="space-y-3">
              <div className="flex justify-center gap-2">
                {activeModule.steps.map((_, i) => (
                  <div
                    key={i}
                    className={`h-1.5 rounded-full transition-all duration-500 ${i <= currentStep ? 'bg-[#E2FF6F] w-6' : 'bg-white/10 w-2'}`}
                  />
                ))}
              </div>
        ...
        <>
            ...
            <div className="max-w-2xl mx-auto flex items-center justify-between">
              ...
              {locationStatus === 'denied' ? (
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-white/10 bg-white/5 text-xs font-bold text-white/60"
                >
                ...
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 text-emerald-400 text-xs font-bold">
                  <span className="w-2 h-2 rounded-full bg-emerald-400" /> Located
                </div>
              )}
            </div>
          </div>
          ...
          {isAlertToastVisible && (
            <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[110] bg-rose-500/20 border border-rose-500/30 text-rose-300 px-6 py-3 rounded-2xl text-sm font-bold shadow-2xl backdrop-blur-xl">
          ...
          <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-6 mt-2">
            <section className="space-y-4">
              <a
                href="tel:911"
                className="w-full bg-[#111] hover:bg-[#1a1a1a] text-white p-6 rounded-3xl shadow-2xl border-2 border-rose-600/30 flex items-center justify-between active:scale-[0.97] transition-all"
              >
                <div>
                  <div className="text-4xl font-black tracking-tighter text-rose-500">911</div>
                  <div className="text-white/60 mt-1 font-bold tracking-widest text-sm uppercase">
                    Emergency Services
                  </div>
                </div>
              ...
              <button
                ...
                className="w-full bg-[#111] hover:bg-[#1a1a1a] text-white p-6 rounded-3xl shadow-2xl border-2 border-[#25D366]/30 flex items-center justify-between transition-all text-left active:scale-[0.97]"
              >
                <div>
                  <div className="text-3xl font-black tracking-tighter text-[#25D366]">
                    TRUSTED ALERT
                  </div>
                  <div className="text-white/60 mt-1 font-bold tracking-widest text-sm uppercase">
                    Send SOS Message
              ...
              <button
                ...
                className="w-full bg-[#111] hover:bg-[#1a1a1a] text-white p-5 rounded-3xl shadow-lg border border-blue-600/30 flex items-center justify-between transition-all text-left active:scale-[0.97]"
              >
                <div>
                  <div className="text-xl font-black tracking-tighter text-blue-400">
                    NEAREST HOSPITAL
                  </div>
                  <div className="text-white/50 mt-1 font-bold tracking-widest text-xs uppercase">
                    Open Map
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {rescueModules.map((m) => (
                  <button
                    ...
                    className="surface-interactive p-4 rounded-2xl text-left border-white/5"
                  >
                    <div
                      className={`w-10 h-10 rounded-xl ${m.color.replace('text', 'bg')}/10 flex items-center justify-center mb-3`}
                    >
            ...
            <section className="space-y-4">
              <details className="surface-card rounded-2xl border border-white/5 overflow-hidden">
                ...
                <div className="p-5 pt-0 border-t border-white/5 text-center">
                  <div className="relative w-40 h-40 mx-auto flex items-center justify-center my-6">
                    ...
                    <div
                      ...
                      style={{
                        width:
                          breathingPhase === 'inhale' || breathingPhase === 'hold' ? '100%' : '60%',
                        height:
                          breathingPhase === 'inhale' || breathingPhase === 'hold' ? '100%' : '60%',
                        transition: `width ${breathingPhase === 'inhale' ? 4 : breathingPhase === 'exhale' ? 8 : 1}s, height ${breathingPhase === 'inhale' ? 4 : breathingPhase === 'exhale' ? 8 : 1}s`,
                      }}
                    >
                      <div className="text-white font-black flex flex-col items-center">
                        ...
                        {breathingPhase !== 'idle' && (
                          <span className="text-4xl font-mono">{timeLeft}</span>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <div
                id="contacts-section"
                className="surface-card p-5 rounded-2xl border border-white/5 space-y-4"
              >
                <div className="flex items-center justify-between">
                  <h2 className="font-bold text-white flex items-center gap-2">
                    <UserPlus className="w-5 h-5" /> Support Network
                  </h2>
                  {!isAddingContact && (
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      className="rounded-lg border-white/10 text-white/60 hover:bg-white/10"
                    >
                ...
                {personalContacts.length === 0 && !isAddingContact && (
                  <div className="p-4 rounded-xl text-center text-sm text-white/50 border border-dashed border-white/10">
                ...
                {personalContacts.map((contact, idx) => (
                  <div
                    key={idx}
                    className="bg-white/5 p-3 rounded-xl flex items-center justify-between"
                  >
                    <div>
                    ...
                    <div className="flex gap-2">
                      <a
                        ...
                        className="h-9 w-9 flex items-center justify-center bg-white/10 rounded-lg"
                      >
                        <Phone className="w-4 h-4" />
                      </a>
                      <button
                        onClick={() => handleRemoveContact(idx)}
                        className="px-2 text-white/50 text-xs hover:text-rose-400"
                      >
                        Del
                      </button>
                    </div>
                  ...
                  <div className="p-4 rounded-xl space-y-3 border border-white/10">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="flex-1 bg-[#E2FF6F] text-black font-bold"
                      >
                        Save
                      </Button>
                      <Button
                ...
                <div className="p-5 pt-0 space-y-4 border-t border-white/5">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      placeholder="When I notice..."
                      className="bg-white/5 border-white/10 rounded-xl text-white placeholder:text-white/40"
                    />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs uppercase tracking-widest font-bold text-white/60">
          ...
          {isAlertModalOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              />
              <div className="relative w-full max-w-sm bg-[#111] rounded-3xl shadow-2xl p-6 border border-white/10">
                <button
                  onClick={() => {
                    setIsAlertModalOpen(false);
                    setSelectedAlertContact(null);
                ...
                <p className="text-sm text-white/60 mb-5">Choose a contact and channel.</p>
                <div className="bg-white/5 text-white/60 text-xs p-4 rounded-xl mb-5 whitespace-pre-wrap">
                ...
                <div className="space-y-2 mb-5">
                  ...
                  {personalContacts.map((c, i) => (
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-xl border text-sm transition-all ${selectedAlertContact?._id === c._id ? 'bg-[#25D366]/10 border-[#25D366]/40 text-white' : 'bg-white/5 border-white/10 text-white/60 hover:border-white/20'}`}
                    >
                      <span className="font-bold">{c.name}</span>
                      <span className="text-xs ml-2 text-white/40">{c.phone}</span>
                    </button>
                  ))}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() =>
                      selectedAlertContact && sendAlert(selectedAlertContact, 'whatsapp')
                    }
                    disabled={!selectedAlertContact}
                    className="py-4 font-bold bg-[#25D366] text-black rounded-2xl hover:bg-[#20bd5a] disabled:opacity-40"
                  >
                    WhatsApp
                  </button>
                  <button
                    ...
                    disabled={!selectedAlertContact}
                    className="py-4 font-bold bg-white text-black rounded-2xl hover:bg-white/80 disabled:opacity-40"
                  >
                    SMS
                  </button>
                </div>
