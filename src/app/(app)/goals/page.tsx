...
export default function GoalsPage() {
  ...
  const progress = store.goals.length === 0 ? 0 : Math.round((store.goals.filter(g => g.completed).length / store.goals.length) * 100);
    ...
    <main className="p-12 max-w-7xl mx-auto space-y-16">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-10">
        <div className="space-y-4">
          ...
          <p className="text-white/40 text-xl font-medium tracking-wide">Define your path to deep structural well-being.</p>
        </div>
          ...
          <div className="glass-panel p-10 flex flex-col md:flex-row items-center gap-12 bg-white/5 border-white/5 shadow-2xl relative overflow-hidden group">
             ...
             <div className="relative w-40 h-40 flex items-center justify-center relative z-10">
                <svg className="w-full h-full -rotate-90">
                  <circle cx="80" cy="80" r="74" stroke="white" strokeOpacity="0.05" strokeWidth="8" fill="transparent" />
                  <motion.circle 
                    cx="80" cy="80" r="74" 
                    stroke="#E2FF6F" 
                    strokeWidth="8" 
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] mt-1">Velocity</span>
                </div>
             </div>
             ...
             <div className="space-y-4 text-center md:text-left relative z-10">
                ...
                <p className="text-white/40 text-lg font-medium max-w-md leading-relaxed">
                  You have successfully operationalized {store.goals.filter(g => g.completed).length} out of {store.goals.length} strategic wellness objectives.
                </p>
                <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                   {categories.map(c => (
                      <div key={c.id} className="flex items-center gap-2.5 px-5 py-2.5 rounded-2xl bg-white/5 border border-white/5 text-[10px] font-bold uppercase tracking-[0.2em] text-white/60 hover:text-[#E2FF6F] hover:border-[#E2FF6F]/30 transition-all">
          ...
          <div className="grid gap-6">
             {store.goals.length === 0 ? (
               <div className="p-24 text-center glass-panel border-white/5 border-dashed border-2 bg-white/[0.02] text-white/20 space-y-6 rounded-[48px]">
                  ...
                  <p className="text-xl font-medium">Your landscape is silent. What is your next mountaintop?</p>
                  <Button variant="outline" size="lg" className="rounded-full border-white/10 text-white/40 hover:text-white" onClick={() => setShowAdd(true)}>Define Initial Goal</Button>
               </div>
             ) : (
                 ...
                 return (
                    ...
                    >
                      <div className="flex items-center gap-8 relative z-10">
                        ...
                        <div>
                          <div className="flex items-center gap-3 mb-2">
                             ...
                             <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#E2FF6F]/40">{cat.label}</span>
                          </div>
                          <h4 className={`font-bold text-2xl tracking-tight transition-all duration-500 ${goal.completed ? "line-through text-white/30" : "text-white group-hover:translate-x-1"}`}>{goal.title}</h4>
                        </div>
                      </div>
                      <div className="flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-x-10 group-hover:translate-x-0 relative z-10">
                         <span className="text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] whitespace-nowrap">FEB 1 – FEB 7</span>
                         <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl bg-white/5 text-white/20 hover:text-rose-500 hover:bg-rose-500/10 transition-all">
        ...
        <div className="space-y-10">
          <div className="glass-panel p-10 space-y-10 bg-white/5 border-white/5 shadow-2xl">
             ...
             <div className="grid grid-cols-2 gap-6">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="aspect-square rounded-3xl bg-white/5 border border-white/5 flex items-center justify-center text-4xl hover:bg-[#E2FF6F]/10 hover:border-[#E2FF6F]/20 transition-all cursor-help relative group overflow-hidden">
                     <span className={i < 3 ? "opacity-100" : "opacity-10 grayscale"}>{i < 3 ? "🏆" : "🔒"}</span>
                     <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 bg-[#0A0C0B]/90 backdrop-blur-md transition-all duration-500 p-4 text-center pointer-events-none border border-[#E2FF6F]/20">
                        <p className="text-[10px] font-bold uppercase text-[#E2FF6F] tracking-widest">Insignia {i}</p>
                        <p className="text-[9px] font-medium leading-relaxed mt-2 text-white/60">{i < 3 ? "Structural Mind" : "Defy 50% threshold"}</p>
                     </div>
                  </div>
                ))}
             </div>
          ...
          <div className="glass-panel p-10 bg-[#E2FF6F]/5 border-[#E2FF6F]/10 space-y-6 relative overflow-hidden group">
            ...
            <h4 className="font-bold text-[#E2FF6F] text-lg tracking-[0.1em] uppercase">Growth Core</h4>
            <p className="text-sm leading-relaxed text-white/50 font-medium relative z-10">
              "Your neural patterns indicate a 22% increase in focus persistence during meditation-heavy cycles. Elite adaptation in progress."
            </p>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showAdd && (
          <div className="fixed inset-0 bg-[#0A0C0B]/95 backdrop-blur-2xl z-50 flex items-center justify-center p-6">
            <motion.div 
            ...
            >
              ...
              <div className="space-y-4 relative z-10">
                <h3 className="text-4xl font-bold tracking-tighter text-white">Define Objective</h3>
                <p className="text-white/40 font-medium italic">What structural change shall we manifest today?</p>
              </div>

              <div className="space-y-10 relative z-10">
                <div className="space-y-4">
                  <label className="text-[10px] font-bold text-[#E2FF6F] uppercase tracking-[0.3em]">Objective Title</label>
                  <input 
                    autoFocus
                    value={newTitle}
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-bold text-[#E2FF6F] uppercase tracking-[0.3em]">Focus Domain</label>
                  <div className="grid grid-cols-3 gap-4">
                    {categories.map(c => (
                      <button
                        key={c.id}
                        onClick={() => setNewCat(c.id as any)}
              ...
              <div className="flex gap-4 pt-8 relative z-10">
                ...
                <Button className="flex-1 h-16 rounded-[24px] bg-[#E2FF6F] hover:bg-[#d4f056] text-black font-bold tracking-widest uppercase shadow-xl shadow-[#E2FF6F]/10 active:scale-95 transition-all" onClick={handleAdd}>Confirm Objective</Button>
              </div>
            </motion.div>
          </div>
        )}
