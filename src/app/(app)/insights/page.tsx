...
import { PieChart, TrendingUp, Zap, Moon, Wind, BookOpen, Activity } from 'lucide-react';
import { useMemo, lazy, Suspense } from 'react';
import { Mood } from '@/lib/types';
import { motion } from 'framer-motion';

const InsightsSleepChart = lazy(() => import('@/components/charts/InsightsCharts'));
const InsightsJournalChart = lazy(() => import('@/components/charts/InsightsJournalChart'));
...
export default function InsightsPage() {
  ...
  const habitCompletionRate = activeHabitsCount
    ? Math.round((completedTodayCount / activeHabitsCount) * 100)
    : 0;

  return (
    <main className="p-4 md:p-8 lg:p-12 max-w-7xl mx-auto space-y-8 md:space-y-12">
      <header className="space-y-4 pt-4 md:pt-0">
        ...
        <p className="text-white/40 text-lg md:text-xl font-medium tracking-wide">
          Synthesizing data from Mood, Context, Habits, Sleep, Breathing, and Journaling.
        </p>
      </header>
      ...
      {!isDataSufficient ? (
        <div className="glass-panel p-20 text-center flex flex-col items-center justify-center border-white/5 bg-white/5 shadow-2xl relative overflow-hidden rounded-[40px]">
          ...
          <p className="text-white/40 max-w-lg mx-auto text-lg font-medium leading-relaxed relative z-10">
            Engage with any feature across the application (Mood, Sleep, Journals, Habits) to unlock
            your holistic neural dashboard.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="glass-panel p-8 bg-white/5 border-white/5 rounded-[32px] space-y-4 group hover:bg-[#E2FF6F]/5 hover:border-[#E2FF6F]/20 transition-all duration-500">
              <div className="flex items-center gap-3 text-white/50 group-hover:text-[#E2FF6F] transition-colors">
                <Activity className="w-5 h-5" />{' '}
                <span className="font-bold uppercase tracking-widest text-xs">Habit Integrity</span>
              </div>
            ...
            <div className="glass-panel p-8 bg-white/5 border-white/5 rounded-[32px] space-y-4 group hover:border-indigo-400/20 hover:bg-indigo-400/5 transition-all duration-500">
              <div className="flex items-center gap-3 text-white/50 group-hover:text-indigo-400 transition-colors">
                <Moon className="w-5 h-5" />{' '}
                <span className="font-bold uppercase tracking-widest text-xs">Sleep Quality</span>
              </div>
            ...
            <div className="glass-panel p-8 bg-white/5 border-white/5 rounded-[32px] space-y-4 group hover:border-emerald-400/20 hover:bg-emerald-400/5 transition-all duration-500">
              <div className="flex items-center gap-3 text-white/50 group-hover:text-emerald-400 transition-colors">
                ...
                <span className="font-bold uppercase tracking-widest text-xs">Mindfulness</span>
              </div>
              <div className="text-5xl font-black text-white">
                {breathingData[6].mins}
            ...
            <div className="glass-panel p-8 bg-white/5 border-white/5 rounded-[32px] space-y-4 group hover:border-amber-400/20 hover:bg-amber-400/5 transition-all duration-500">
              <div className="flex items-center gap-3 text-white/50 group-hover:text-amber-400 transition-colors">
                <BookOpen className="w-5 h-5" />{' '}
                <span className="font-bold uppercase tracking-widest text-xs">Expression</span>
              </div>
              <div className="text-5xl font-black text-white">
                {journalData[6].words}
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="glass-panel p-10 bg-white/5 border-white/5 shadow-2xl rounded-[40px] space-y-8">
              <h3 className="font-bold text-2xl text-white tracking-tight flex items-center gap-4">
                <Moon className="w-6 h-6 text-indigo-400" /> Somatic Recovery (Sleep)
              </h3>
              <div className="h-[250px] w-full">
                <Suspense fallback={<div className="h-full animate-pulse bg-white/5 rounded-xl" />}>
                  <InsightsSleepChart sleepHistory={store.sleepHistory} />
                </Suspense>
              </div>
            </div>
            ...
            <div className="glass-panel p-10 bg-white/5 border-white/5 shadow-2xl rounded-[40px] space-y-8">
              <h3 className="font-bold text-2xl text-white tracking-tight flex items-center gap-4">
                <BookOpen className="w-6 h-6 text-amber-500" /> Expression & Clarity (Journaling)
              </h3>
              <div className="h-[250px] w-full">
                <Suspense fallback={<div className="h-full animate-pulse bg-white/5 rounded-xl" />}>
                  <InsightsJournalChart journalEntries={store.journalEntries} />
                </Suspense>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="glass-panel p-10 bg-white/5 border-white/5 shadow-2xl rounded-[40px] space-y-10 group hover:bg-white/10 transition-all duration-700">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-[#E2FF6F]/20 rounded-2xl group-hover:scale-110 transition-transform">
                  <TrendingUp className="w-6 h-6 text-[#E2FF6F]" />
                </div>
              ...
              <div className="p-8 bg-black/40 rounded-[32px] border border-white/5 group-hover:border-[#E2FF6F]/20 transition-all">
                <p className="text-[10px] font-bold text-white/30 uppercase tracking-[0.3em] mb-4">
                  Dominant Internal State
                </p>
                <div className="flex items-center gap-6">
                  <span className="text-7xl drop-shadow-2xl">{moodEmojis[topMood.mood]}</span>
                  <div>
                    <span className="text-3xl font-bold capitalize text-white block">
                      {topMood.mood}
                    </span>
                  ...
                  .map(([mood, count]) => (
                    <div key={mood} className="space-y-2">
                      <div className="flex items-center justify-between text-xs font-bold tracking-widest uppercase text-white/40">
                        ...
                        <span>
                          {Math.round((count / Math.max(1, store.moodHistory.length)) * 100)}%
                        </span>
                      </div>
                      <div className="h-2.5 bg-black/50 rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{
                            width: `${(count / Math.max(1, store.moodHistory.length)) * 100}%`,
                          }}
            ...
            <div className="glass-panel p-10 bg-gradient-to-br from-[#E2FF6F]/10 to-transparent border-[#E2FF6F]/20 shadow-2xl rounded-[40px] relative overflow-hidden group">
              ...
              <div className="flex items-center gap-4 mb-8 relative z-10">
                <div className="p-3 bg-[#E2FF6F] rounded-2xl scale-90">
              ...
              <p className="text-lg text-white/80 font-medium leading-relaxed relative z-10">
                {journalData[6]?.words > 0 && sleepData[6]?.hours > 0
                  ? `"Your journaling and sleep patterns show strong alignment. Maintaining ${Math.round(sleepData[6]?.hours ?? 0)}h of sleep supports your ${habitCompletionRate}% habit completion rate. Keep the rhythm going."`
                  : sleepData[6]?.hours === 0 && store.sleepHistory.length > 0
                    ? "You haven't logged sleep today yet. Even a short night's rest matters — log it to see your insights improve."
                    : 'Start tracking your mood, sleep, and journaling to unlock personalized AI-powered insights about your wellness patterns.'}
              </p>
            </div>
          </div>
        </div>
