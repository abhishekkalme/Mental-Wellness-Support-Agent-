                ...
                >
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[#E2FF6F] font-bold tabular-nums">{intensity}/10</span>
                    </div>
            ...
            {store.moodHistory.length === 0 ? (
              <div className="h-[400px] flex flex-col items-center justify-center text-center space-y-5">
                ...
                <div>
                  ...
                  <p className="text-white/20 text-sm">Start tracking your mood to see patterns over time.</p>
                </div>
                <button
                  onClick={() => formRef.current?.scrollIntoView({ behavior: 'smooth' })}
                    ...
                    >
                      ...
                      <div className="p-4 rounded-xl bg-white/[0.02] group-hover:bg-white/[0.04] transition-all">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-lg">{moods.find(m => m.type === entry.mood)?.emoji}</span>
                          <p className="font-semibold text-white capitalize">{entry.mood}</p>
                        </div>
                        <p className="text-xs text-white/30">
                          {format(new Date(entry.timestamp), 'MMM d • h:mm a')}
                        </p>
                        {entry.notes && <p className="text-sm mt-2 text-white/50 leading-relaxed">{entry.notes}</p>}
                        {entry.activities && entry.activities.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 mt-2">
                            {entry.activities.map(a => (
                              <span key={a} className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-white/[0.04] text-white/30 border border-white/[0.06]">{a}</span>
                            ))}
                          </div>
                        )}
                      </div>
